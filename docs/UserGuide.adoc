...
Sorts the addressbook by address or by name reverts back to unsorted state upon app reboot. +
Format: `sort OPTION`

***
* OPTION keyword allows to select to sort by name or address.
* To sort by name use `/n`. Example `sort /n`.
* To sort by address use `/a`. Example `sort /a`.
***
...
* `find 111 222 333` +
Returns any person having names `1111`, `0222`, or `9333`
...
=======
Format: `letter KEYWORD [One Capital letter character]`

****
* The search is not case insensitive. e.g `A` will only match `A`
* Only the name is searched.
* Only first character of the word will be matched e.g. `A` will match `Alan`
*****

Examples:

* `letter A` +
Returns `Alan` and `Alex Doe`
